...
// ─────────────────────────────────────────────────────────────
// Draft storage: IndexedDB (large quota) with sessionStorage fallback.
//
// sessionStorage caps out at ~5MB per origin. A single uploaded photo
// becomes a ~2–3MB base64 data URL inside the layer, so saving drafts
// for both sides easily exceeds the quota — the second save then fails
// silently and the layer "disappears" when the user comes back to that
// side. IndexedDB has a much higher quota (typically hundreds of MB).
// ─────────────────────────────────────────────────────────────
...
function saveCurrentSideDraft() {
  // Snapshot synchronously so the saved data reflects the moment of the call,
  // even if a navigation is queued immediately after.
  ...
  pendingSavePromise = pendingSavePromise.then(async () => {
    try {
      ...
      // One-time cleanup: remove any legacy sessionStorage entry for this key
      // so we don't keep stale data around after migration.
      try { sessionStorage.removeItem(key); } catch (_) {}
    } catch (error) {
      // IndexedDB unavailable or write failed — try sessionStorage as last resort.
      try {
...
async function changeConstructorVariant(variantId) {
  // IndexedDB writes are async; we MUST wait for the draft to persist
  // before navigating, otherwise the in-flight write is cancelled by page unload
  // and the current side's design is lost.
...
function loadPendingAiPrint() {
  ...
  try {
    ...
    // Corrupt payload — discard so it doesn't keep blocking subsequent runs.
  ...
  // Only consume the handoff when product/variant match the current constructor.
  // Otherwise leave it in storage so the user is taken to the right page next time.
...
loadProductMockup();
resizeCanvas();
// Restore draft first, THEN apply pending AI print on top: otherwise the AI
// layer can be added and immediately wiped out when the draft restore replaces
// the in-memory layers array.
async function restoreCurrentSideDraftThenLoadPendingAiPrint() {
  try {
    await restoreCurrentSideDraft();
  } finally {
    loadPendingAiPrint();
  }
}
restoreCurrentSideDraftThenLoadPendingAiPrint();
</script>
</body>
</html>
