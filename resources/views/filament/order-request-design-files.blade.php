@php
    $designs = $record->items->flatMap(fn ($item) => $item->designs);
@endphp
        ...
        @foreach ($designs as $design)
            <div class="rounded border border-gray-200 p-4">
                <div class="mb-3 text-base font-semibold">
                    {{ $design->side === 'back' ? 'Сзади' : 'Спереди' }}
                    <span class="text-xs font-normal text-gray-500">#{{ $design->id }}</span>
                </div>
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    @if ($design->preview_image_path)
                        <div>
                            ...
                            <img src="{{ Storage::disk('public')->url($design->preview_image_path) }}" alt="Preview {{ $design->side }}" class="max-h-96 rounded border object-contain">
                    ...
                    <div>
                        <div class="mb-2 text-sm font-medium">Print-only</div>
                        ...
                        @else
                            <div class="text-sm text-gray-500">Print-only файл не найден.</div>
                        @endif
                    </div>
                </div>
